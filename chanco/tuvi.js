...
//		ctx.fillText(cung.LuuTieuHanTen, locx + width/4 - 60, locy + height/4 - 20);
	...
	if(cung.ThangHan != 13){
		...
		if(g_isNom == 1){
			if(g_isFullOpt == 0 && g_isLocNhap == 0) ctx.fillText("月 " + cung.ThangHan, locx + width/4 - 35, locy + height/4 - 20);
...
//	stt++;
	...
	var tggg = "";	
	switch(hieuso){
		case 0: tggg = "Mệnh Cục bình hòa";break;
		case 1: tggg = "Mệnh sinh Cục";break;
		case 4: tggg = "Cục sinh Mệnh";break;
		case 2: tggg = "Mệnh khắc Cục";break;
		case 3: tggg = "Cục khắc Mệnh";break;
		default : tggg = "";
	}
	if(g_isNom == 1){
		switch(hieuso){
			case 0: tggg = "命 局 平 和";break;
			case 1: tggg = "命 生 局";break;
			case 4: tggg = "局 生 命";break;
			case 2: tggg = "命 剋 局";break;
			case 3: tggg = "局 剋 命";break;
			default : tggg = "";
		}		
	}
	ctx.textAlign="center";		
	...
	ctx.fillText("Mời các bạn lấy lá số tại http://tuvichanco.vn", width/2, 3*height/4 - 16);
	ctx.textAlign="left";	
	ctx.fillStyle = "#000000";
	ctx.stroke();
}
...
//	stt++;
	...
	switch(hieuso){
		case 0: tg = "Mệnh Cục bình hòa";break;
		case 1: tg = "Mệnh sinh Cục";break;
		case 4: tg = "Cục sinh Mệnh";break;
		case 2: tg = "Mệnh khắc Cục";break;
		case 3: tg = "Cục khắc Mệnh";break;
		default : tg = "";
	}

	if(g_isNom == 1){
		switch(hieuso){
			case 0: tg = "命 局 平 和";break;
			case 1: tg = "命 生 局";break;
			case 4: tg = "局 生 命";break;
			case 2: tg = "命 剋 局";break;
			case 3: tg = "局 剋 命";break;
			default : tg = "";
		}		
	}
	...
	ctx.fillText("Mời các bạn lấy lá số tại http://tuvichanco.vn", width/2, 3*height/4 - 32);
...
function showHidden(){
	...
	tex += '<td align="center" colspan="2">Họ và Tên: <input id="hovaten" name="hoten" type="text" value="' + g_hoten + '" size="25" maxlength="25"/></td>';
	tex += '</tr><tr><td>';
...
//	tex += '<td >Họ và Tên:</td><td><input id="hovaten" name="hoten" type="text" value="' + g_hoten + '" size="25" maxlength="25"/></td>';
	...
	tex += '<td><input name="isNam" type="radio" value="1" ' + tg1 + ' onClick="applyIsNam(1);"/> Nam </td><td> <input name="isNam" type="radio" value="0" ' + tg2 + ' onClick="applyIsNam(0);"/> Nữ</td>';	
	tex += '</tr><tr>';
	if(g_isduonglich == 1){
		tg1 = 'checked="checked" ';
		tg2 = "";
	...
	tex += '<td><input name="duonglich" type="radio" ' + tg1 + 'value="1" onClick="changeAmlich(1);"/> Dương Lịch </td><td> <input name="duonglich" type="radio" value="0" ' + tg2 + ' onClick="changeAmlich(0);"/> Âm lịch</td>';
	tex += '</tr><tr>';
	tex += '<td>Năm Sinh:</td><td><input type="text" id="namsinh" name="namsinh" maxlength="10" onKeyUp="changeYear();" size="10" value="' + g_namSinh + '"/></td>';
	tex += '</tr><tr>';	
	...
	tex += '<td><input type="radio" name="anhMau" ' + tg1 + ' onClick="applyIsMau(1);"/> Màu</td><td><input type="radio" name="anhMau" ' + tg2 + ' onClick="applyIsMau(0);" /> Đen trắng</td>';
	...
	tex +=	'<td colspan="2"><input type="checkbox" ' + tgg + ' name="checkTtinh" id="checkTtinh" value="1"/> An vòng Tướng tinh </td>';
	tex += '</tr><tr>';		
	
	if(g_isLuuHan == 0) tgg = "";
	else tgg = ' checked="checked" ';		
	tex += '<td colspan="2"><input type="checkbox" ' + tgg + ' name="checkHan" value="1" onClick="changeCheckNamHan();"/> Hạn năm: ';
	...
	tex +=	'<td><input type="checkbox" ' + tgg + ' name="checkLuuDaiVan" id="checkLuuDaiVan" value="1" /> Lưu đại vận </td>';
	
	if(g_isLuuKhoiVietKhac == 0) tgg = "";
	else tgg = ' checked="checked" ';
	tex +=	'<td><input type="checkbox" ' + tgg + ' name="checkLuuKhoiVietKhac" id="checkLuuKhoiVietKhac" value="1"/> Lưu các sao khác </td>';
	tex +=	'</tr><tr>';
		
	if(g_isLocNhap == 0) tgg = "";
	else tgg = ' checked="checked" ';
	tex +=	'<td><input type="checkbox" ' + tgg + ' name="checkLocNhap" id="checkLocNhap" value="1" /> Lộc Kỵ nhập </td>';

	if(g_isLuuTuanTriet == 0) tgg = "";
	else tgg = ' checked="checked" ';
	tex +=	'<td><input type="checkbox" ' + tgg + ' name="checkLuuTuanTriet" id="checkLuuTuanTriet" value="1" /> Lưu Tuần Triệt </td>';
	...
	tex += '<td><input type="radio" name="luuthaitue" ' + tg1 + ' onClick="g_isLuuThaiTueDB = 1;"/> Địa Bàn </td><td><input type="radio" name="luuthaitue" ' + tg2 + ' onClick="g_isLuuThaiTueDB = 0;" /> Tiểu Hạn </td>';
	...
	tex += '<td><input type="radio" name="isNom" ' + tg1 + ' onClick="g_isNom = 0;"/> Việt </td><td><input type="radio" name="isNom" ' + tg2 + ' onClick="g_isNom = 1;" /> Nôm </td>';
...
//	tex += '<button onClick="download();">Tải ảnh xuống</button>';
	tex += ' <a href="#" id="downloadLnk" download="' + g_hoten.trim() + '.jpg"> <button>Tải ảnh về</button></a><br/><br/>';
...
//	style="width: 40px"	
	...
	tex = "";	
	if(g_isduonglich == 1){
		tex += '<input type="number" id="giosinhH" value="' + g_gioDH + '" min="0" max="23" maxlength="2" size = "2" /> giờ ';
		tex += '<input type="number" id="giosinhM" value="' + g_gioDM + '" min="0" max="59" maxlength="2" size = "2" /> phút';
	}else{
